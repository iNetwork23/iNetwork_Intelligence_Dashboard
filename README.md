...
- Supabase-Objekte, RLS/Grants, Identitätsconstraint und eine scoped Kohortenstichprobe sind verifiziert. Die genehmigte LTV-Jobkorrektur wurde am 7. September um 09:52:57 UTC angewendet (Migration 20260907095257): 15-Minuten-Budget, unverändert Minute 25; der nächste reguläre Lauf wird gesondert verifiziert. [Hydrierungs-/Speicherpaket und Abnahmegrenzen](docs/WLX-CLOSURE-2026-09-07.md). Backfill-/Datenparität, vollständige Rollen-/Browsermatrix und kontrollierte Providerabläufe bleiben offen.
...
- Nachfolgende authentifizierte Prüfung: [Kohorten-Navigation, Fraud-Timeout, Geldbeträge und Hydrierung von Startseite/Zeitraum/Seitenleiste](docs/WLX-BROWSER-ACCEPTANCE-2026-09-07.md). Das Hydrierungspaket besteht 196 Testdateien / 1599 Tests; vollständige Projektabnahme bleibt offen. Der anschließend beobachtete Fraud-OOM und das begrenzte Auswertungspaket sind im [Fraud-/LTV-Nachweis](docs/WLX-FRAUD-MEMORY-LTV-2026-09-07.md) dokumentiert.
...
3. Den zusätzlichen Leseindex `20260906215000_index_affiliate_conversion_reads.sql` nur nach dem exakten Scope in `docs/WLX-DATABASE-READBACK-2026-09-06.md` und ausdrücklicher Freigabe einzeln über einen Autocommit-Kanal aufbauen. Die Produktions-Migrationshistorie enthält seit 7. September 09:52:57 UTC die gezielte LTV-Jobkorrektur; ältere vorhandene Objekte sind dort weiterhin nicht vollständig registriert. Niemals pauschal alle Dateien erneut ausführen.
   `20260907095257_repair_ltv_cron_statement_budget.sql` korrigiert ausschließlich den bestehenden Stundenjob nach Prüfung seines aktuellen Befehls und ausdrücklicher Freigabe. Zeitplan bleibt `25 * * * *`; kein Sofortlauf. Read-back: Jobbefehl, nächster Lauf und `sync_state.ltv_cohorts_materialized` müssen tatsächlich Erfolg bestätigen. Rückweg: `docs/sql/rollback-ltv-cron-statement-budget.sql`.
   Die vorbereitete `20260908090600_bound_conversion_replacement_timeout.sql` setzt nach erneutem Read-back und eigener ausdrücklicher Freigabe ausschließlich das Zeitbudget von `replace_conversion_window` auf 45 Sekunden, mit fünf Sekunden Locktimeout. Sie wird nicht durch das App-Deployment angewendet. Scope und Read-back: `docs/WLX-IMPORT-READBACK-2026-09-08.md`; Rückweg: `docs/sql/berlin-reporting-20260908/03-restore-conversion-timeout.sql`. Danach muss ein begrenzter Fraud-Lauf seine tatsächliche Identitäts-/Typ-Parität bestätigen.
   `20260909104000_add_approved_conversion_read_view.sql` ergänzt nach eigener ausdrücklicher Freigabe die ausschließlich für `service_role` lesbare View `affiliate_approved_conversions`. Zuerst Migration, Definition, Rechte und Datenparität gemäß `docs/WLX-APPROVED-CONVERSION-VIEW-2026-09-09.md` bestätigen; erst danach den davon abhängigen Anwendungscode deployen. Rückweg: zuerst vorherigen Anwendungscode wiederherstellen, anschließend `docs/sql/rollback-approved-conversion-read-view.sql`. Kein automatischer Migrationslauf beim App-Deployment.
