...
--
-- This table intentionally omits source_id and sub_source. The Smartlink views
...
-- ACHTUNG: In der Zieldatenbank existiert bereits eine fremde Tabelle public.hourly_metrics
-- mit der Zeitspalte hour_start sowie sync_revision und raw. Sie stammt nicht aus diesem
-- Repository und wird offenbar von einem anderen System geschrieben. Diese Datei darf sie
-- deshalb weder droppen noch verändern. Der endgültige Tabellenname wird festgelegt, sobald
-- geklärt ist, ob die vorhandene Tabelle mitgenutzt oder eine eigene angelegt wird.
...
create index if not exists hourly_metrics_hour_idx on public.hourly_metrics (metric_hour);
create index if not exists hourly_metrics_campaign_hour_idx on public.hourly_metrics (campaign_id, metric_hour);
create index if not exists hourly_metrics_affiliate_hour_idx on public.hourly_metrics (affiliate_id, metric_hour);
...
revoke all on public.hourly_metrics from anon, authenticated;
grant all on public.hourly_metrics to service_role;
...
create or replace function public.prune_hourly_metrics(p_before timestamptz)
returns bigint
language plpgsql volatile security invoker set search_path = public as $$
declare removed bigint;
begin
  delete from public.hourly_metrics where metric_hour < p_before;
  get diagnostics removed = row_count;
  return removed;
end;
$$;

revoke all on function public.smartlink_hourly_rows(timestamptz,timestamptz,text[],text), public.prune_hourly_metrics(timestamptz) from public, anon, authenticated;
grant execute on function public.smartlink_hourly_rows(timestamptz,timestamptz,text[],text), public.prune_hourly_metrics(timestamptz) to service_role;
