...
import type{SourceCandidateRange}from'@/lib/source-candidate-link';
import{BULK_BLOCK_LIMIT,buildSourceCandidateQuery,firstSaleRate,maturityLabel,selectSourceCandidates,SOURCE_CANDIDATE_PAGE_SIZE,toggleBulkSelection,verdictLabel,type SourceCandidateBlockState,type SourceCandidateFilters,type SourceCandidateRow,type SourceCandidateSort}from'@/lib/source-candidate-view';
...
const day=(value:string)=>new Intl.DateTimeFormat('de-DE',{dateStyle:'medium',timeZone:'Europe/Berlin'}).format(new Date(value));
const blockLabel=(block:SourceCandidateBlockState)=>block.status==='active'?`Gesperrt seit ${day(block.effectiveAt)}`:block.status==='pending'?'Verifizierung läuft':'Zustand unklar';
...
export default function SourceCandidateList({rows:initialRows,range,openKey,initialFilters,initialSort,mayBlock,finance,blockStatusUnknown=false}:Props){
 ...
 const selection=useMemo(()=>selectSourceCandidates(rows,filters,sort,limit,openKey),[rows,filters,sort,limit,openKey]);
 ...
 const setBlock=useCallback((key:string,block:SourceCandidateBlockState)=>{setRows(current=>current.map(row=>row.key===key?{...row,block}:row));setSelected(current=>current.filter(item=>item!==key))},[]);
 ...
 return <section className="sourcesPanel">
  <div className="sourcesToolbar">
   <form className="sourcesFilters" onSubmit={event=>event.preventDefault()} aria-label="Filter der Quellenliste">
    <label>Aktion<select value={filters.action} onChange={event=>update({action:event.target.value as SourceCandidateFilters['action']})}><option value="all">Alle</option><option value="ABSCHALTEN">{verdictLabel('ABSCHALTEN')}</option><option value="SKALIEREN">SKALIEREN</option><option value="BEOBACHTEN">BEOBACHTEN</option></select></label>
  ...
  <div className="tableWrap sourcesTableWrap"><table className="performanceTable sourcesTable"><caption className="srOnly">Partnerübergreifende Quellen mit Handlungsbedarf</caption>
   ...
   <tbody>{selection.rows.map((row,index)=>{const isOpen=row.key===openKey,checked=selected.includes(row.key);return <tr key={row.key} id={row.domId} className={isOpen?'sourcesOpenRow':undefined} aria-current={isOpen?'true':undefined}>
    <td data-label={mayBlock?'Auswahl':'#'}>{mayBlock&&!row.block?<label className="sourcesSelect"><input type="checkbox" checked={checked} onChange={()=>toggle(row.key)} aria-label={`${row.affiliate} · ${row.offer} · ${row.mainValue||'nicht übermittelt'} auswählen`}/></label>:integer(index+1)}</td>
    ...
    <td data-label="Sperrstatus">{blockStatusUnknown?'–':row.block?<><b className={`sourcesBlocked ${row.block.status}`}>{blockLabel(row.block)}</b>{mayBlock&&<InstantLink href="/source-blocks" className="sourcesBlockLink">Sperre ansehen</InstantLink>}{row.block.status==='error'&&mayBlock&&<div className="sourcesRowAction"><SourceBlockButton affiliateId={row.affiliateId} affiliateName={row.affiliate} offerId={row.offerId} offerName={row.offer} trafficMode={row.trafficMode} level={row.level} mainValue={row.mainValue} subValue={row.subValue} showMoney={finance}/></div>}</>:mayBlock?<div className="sourcesRowAction"><SourceBlockButton affiliateId={row.affiliateId} affiliateName={row.affiliate} offerId={row.offerId} offerName={row.offer} trafficMode={row.trafficMode} level={row.level} mainValue={row.mainValue} subValue={row.subValue} showMoney={finance} metrics={{payout:row.payout,sois:row.sois,profit:row.profit,clicks:row.clicks,firstSales:row.firstSales,maturity:maturityLabel(row),leadStatus:row.leadStatus}} onBlocked={records=>{const record=records.find(item=>item.offerId===Number(row.offerId))??records[0];if(record)setBlock(row.key,blockStateFromRecord(record))}}/></div>:'Nicht gesperrt'}</td>
   </tr>})}
  ...
  {bulkOpen&&selectedRows.length>0&&<SourceBulkBlockDialog rows={selectedRows.slice(0,BULK_BLOCK_LIMIT)} finance={finance} onClose={closeBulk} onBlocked={setBlock}/>}
 </section>;
}
