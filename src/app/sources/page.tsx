...
export default async function SourcesPage({searchParams}:{searchParams:Promise<Params>}){
 ...
 const rows=snapshot?prepareSourceCandidateRows(snapshot.rows,index,{finance,access:user.access}):[];
 ...
 return <LocalizedMain className="dashboard sourcesPage"><DashboardPageHeader kicker="Traffic-Kontrolle" title="Quellen" status={header.label} tone={header.tone} icon="affiliate" description="Partnerübergreifende Quellen mit Handlungsbedarf – Verdikt, Volumen und Sperrstatus in einer Liste, Sperre direkt aus der Zeile."/>
  ...
  {rangeSwitch}
  {snapshot?<>
   <p className="sourcesRollup" role="status">Rollup vom {berlinDateTime(snapshot.generatedAt)} · {snapshot.scopeRestricted?'Zugewiesener Bereich':`${snapshot.affiliatesProcessed} von ${snapshot.affiliates} Partnern`} · {rows.length} Kandidaten</p>
   ...
   {snapshot.maturityUnavailable?<section className="sourcesWarning" role="status"><strong>Reife nicht prüfbar</strong><span>{snapshot.scopeRestricted?'Die Reifeprüfung ist noch nicht vollständig verfügbar.':`Für ${snapshot.maturityUnavailable} Partner konnten die Conversions nicht geladen werden – deren Ausschalt-Kandidaten stehen fail-closed auf BEOBACHTEN.`}</span></section>:null}
   {!snapshot.coverageComplete&&<section className="sourcesWarning" role="alert"><strong>Rollup unvollständig</strong><span>{snapshot.scopeRestricted?'Die Vollständigkeit der Quellenliste ist noch nicht bestätigt.':`${snapshot.affiliatesProcessed} von ${snapshot.affiliates} Partnern wurden innerhalb des Zeitbudgets ausgewertet. Quellen fehlender Partner sind nicht bewertet und fehlen in dieser Liste.`}</span></section>}
