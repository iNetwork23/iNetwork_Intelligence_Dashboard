...
import {localizeLinkContent} from '../components/LocalizedLinkContent';
...
function PickerLink({children,...props}:ComponentProps<typeof Link>){const locale=useHydratedLocale();return <Link {...props} data-no-translate>{localizeLinkContent(children,locale)}</Link>}
...
export default function CampaignPicker({campaigns,currentId,affiliateId,returnTo,initialQuery='',initialPartner,initialOpen,associationError=''}:Props){
 ...
 return <LocalizedRoot><section className="campaignPicker partnerCampaignPicker">
  <div className="pickerLabel"><div><b>SMARTLINKS UND PARTNER</b><span>{campaigns.length} Campaigns · beobachtete Zuordnung der letzten 30 Tage</span></div>{currentId&&<PickerLink href={returnTo||'/affiliates?mode=smartlinks'}>Alle Smartlinks anzeigen</PickerLink>}</div>
  ...
  <div className="campaignDropdown partnerCampaignDirectory">{filtered.map(campaign=>{const expanded=openId===campaign.network_campaign_id,selectedPartner=effectivePartner!=='all'&&effectivePartner!=='unassigned'?campaign.partners.find(item=>item.id===effectivePartner):campaign.partners.length===1?campaign.partners[0]:undefined,incomingPartner=campaign.partners.find(item=>item.id===affiliateId),analysisAffiliate=selectedPartner?.id||incomingPartner?.id;return <article className={`partnerCampaignRow${campaign.network_campaign_id===currentId?' current':''}`} key={campaign.network_campaign_id}>
   <button type="button" className="partnerCampaignSummary" aria-expanded={expanded} aria-controls={`campaign-details-${campaign.network_campaign_id}`} onClick={()=>toggle(campaign.network_campaign_id)}><span className="campaignIdentity"><small>CAMPAIGN #{campaign.network_campaign_id}</small><strong data-no-translate>{campaign.campaign_name}</strong><em>{campaign.campaign_status} · {campaign.activeLandingpageCount} aktive LP-Slots</em></span><span className="campaignPartners">{associationError?<b className="mappingUnknown">Zuordnung nicht verfügbar</b>:campaign.partners.length?campaign.partners.map(item=><b key={item.id}><span data-no-translate>{item.name}</span><small>Affiliate #{item.id}</small></b>):<b className="unassigned">Partner nicht zugeordnet</b>}</span><i aria-hidden="true">{expanded?'−':'+'}</i></button>
   {expanded&&<div className="partnerCampaignDetails" id={`campaign-details-${campaign.network_campaign_id}`}>
    <section><div><span>PARTNERZUORDNUNG</span><h3>Beobachtete Partnerzuordnung · letzte 30 Tage</h3></div>{associationError?<p className="mappingUnknown">{associationError}</p>:campaign.partners.length?<div className="partnerAssignmentList">{campaign.partners.map(item=><article key={item.id}><div><b data-no-translate>{item.name}</b><small>Affiliate #{item.id}</small></div><PickerLink href={statefulHref(campaign.network_campaign_id,item.id,returnTo,query,item.id,campaign.network_campaign_id)}>Partner im Affiliate Optimizer öffnen</PickerLink></article>)}</div>:<p>Im 30-Tage-Beobachtungsfenster wurde kein Affiliate-Traffic gefunden. Das ist keine behauptete Eigentümerzuordnung.</p>}</section>
    <section><div className="campaignTechnical"><span><small>Campaign</small><b>#{campaign.network_campaign_id} · {campaign.campaign_status}</b></span><span><small>Tracking-Domain</small><b>{campaign.network_tracking_domain_id?`#${campaign.network_tracking_domain_id}`:'nicht hinterlegt'}</b></span><span><small>Aktuelle Slots</small><b>{campaign.redirects.length}</b></span></div>{campaign.redirects.length?<div className="campaignRedirectList">{campaign.redirects.map(slot=><article key={`${slot.offerId}-${slot.offerUrlId}`}><span><small>LP #{slot.offerUrlId} · Offer #{slot.offerId}</small><b data-no-translate>{slot.name}</b></span><span><b>{slot.weight.toLocaleString('de-DE')} %</b><small>{slot.status}</small></span></article>)}</div>:<p>Keine Redirect-Slots im Campaign-Snapshot.</p>}</section>
    <div className="campaignDetailActions"><PickerLink href={analysisAffiliate?statefulHref(campaign.network_campaign_id,analysisAffiliate,returnTo,query,effectivePartner,campaign.network_campaign_id):legacyFallbackHref(campaign.network_campaign_id,returnTo,query,effectivePartner)}>Im Affiliate Optimizer analysieren</PickerLink>{selectedPartner&&<PickerLink href={statefulHref(campaign.network_campaign_id,selectedPartner.id,returnTo,query,selectedPartner.id,campaign.network_campaign_id)}>Partner im Affiliate Optimizer öffnen</PickerLink>}</div>
   </div>}
