...
const noBalanceReason=(block:SourceBlockRecord)=>!block.metricsAtBlock?'ohne Referenzwerte gesperrt (vor Einführung der Bilanz)':'keine SOIs im Referenzfenster';
...
export default async function SourceBlocksPage({searchParams}:{searchParams:Promise<Params>}){
 ...
 return <LocalizedMain className="dashboard sourceBlocksPage"><DashboardPageHeader kicker="Traffic-Kontrolle" title="Sperr-Bilanz" status={`${blocks.filter(block=>block.status==='active').length} aktiv`} tone="live" icon="automation" description="Offer-spezifische Payout- und Postback-Sperren mit Verstoßsummen, stündlichem Everflow-Abgleich und lückenloser Historie."/>
  ...
  <p className="sourceBlockNotice">Bilanz je Sperre: „Vermiedener Payout“ ist die Vergütung, die für die seit der Sperre gezählten SOIs zum Payout je SOI der 30 Tage vor der Sperre angefallen wäre; „Entgangener Umsatz“ analog mit dem Umsatz je SOI davor; Bilanz = Differenz (positiv: die Sperre lohnt sich).</p>
  ...
  <section className="sourceBlockAuditList">{visible.length?visible.map(block=>{const effect=effectById.get(block.id),marker=markers.get(block.id);return <article key={block.id} className={`sourceBlockAuditCard ${block.status}`}><header><div><span>{block.status==='active'?'GESPERRT':block.status==='pending'?'VERIFIZIERUNG LÄUFT':block.status==='error'?'ZUSTAND UNKLAR':'WIEDER AKTIV'}</span><h2 data-no-translate>{block.affiliateName} · {block.offerName}</h2></div><SourceBlockButton affiliateId={String(block.affiliateId)} affiliateName={block.affiliateName} offerId={String(block.offerId)} offerName={block.offerName} trafficMode={block.trafficMode} level={block.level} mainValue={block.mainValue} subValue={block.subValue}/></header>
   <dl><div><dt>Partner</dt><dd data-no-translate>{block.affiliateName} · #{block.affiliateId}</dd></div><div><dt>Offer</dt><dd data-no-translate>{block.offerName} · #{block.offerId}</dd></div><div><dt>Quelle</dt><dd>{block.mainField}={block.mainValue?<span data-no-translate>{block.mainValue}</span>:'nicht übermittelt'}{block.level==='sub_source'?<span data-no-translate>{` · ${block.subField}=${block.subValue}`}</span>:' · alle Unterquellen'}</dd></div>
    <div><dt>Kategorie</dt><dd>{block.reasonCategory?SOURCE_BLOCK_REASON_LABELS[block.reasonCategory]:'ohne Kategorie'}</dd></div><div><dt>Begründung</dt><dd data-no-translate>{block.reason||'–'}</dd></div><div><dt>Gesperrt seit</dt><dd>{fmt(block.effectiveAt)}</dd></div>
