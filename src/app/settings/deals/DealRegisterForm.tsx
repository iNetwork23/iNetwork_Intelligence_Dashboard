 ...
 return <LocalizedRoot><section className="dealRegister" aria-label="Deal-Register">
  ...
  <form className="dealRegisterForm" onSubmit={upsert} aria-label={editingKey?'Regel ändern':'Regel anlegen'}>
   ...
   <div className="dealRegisterButtons">
    ...
    {editingKey&&<button type="button" className="secondary" onClick={()=>{setDraft(emptyDraft());setEditingKey(null)}} disabled={busy||unavailable}>Abbrechen</button>}
    <button type="button" className="secondary" onClick={applyDefaults} disabled={busy||unavailable}>Standardregeln übernehmen</button>
    <button type="button" className="primary" onClick={save} disabled={busy||!dirty||saveBlocked}>{busy?'Speichern …':'Register speichern'}</button>
