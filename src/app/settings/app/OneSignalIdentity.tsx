...
export default function OneSignalIdentity({enabled,appId,safariWebId,externalId}:{enabled:boolean;appId:string;safariWebId:string;externalId:string}){
 useEffect(()=>{
  ...
  if(enabled&&externalId&&(window.__wlxOneSignalBinding===desired||window.__wlxOneSignalBinding===`pending:${desired}`))return;
  ...
  window.OneSignalDeferred.push(async function(OneSignal){
   try{
    ...
    if(!enabled||!externalId){await OneSignal.logout();delete window.__wlxOneSignalBinding;return}
    await bindOneSignalIdentity(OneSignal,{previousBinding,desiredBinding:desired,externalId});window.__wlxOneSignalBinding=desired;
   }catch{
