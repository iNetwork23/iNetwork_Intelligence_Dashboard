...
export default function SourceBlockProviderPreviewPanel({identity,disabled=false}:{identity:SourceBlockInput;disabled?:boolean}){
 ...
 const [preview,setPreview]=useState<SourceBlockProviderPreview|null>(null),[loading,setLoading]=useState(false),[error,setError]=useState<'history'|'provider'|null>(null);
 useEffect(()=>()=>controller.current?.abort(),[]);
 const read=async()=>{
  if(loading||disabled)return;
  const abort=new AbortController();controller.current=abort;setLoading(true);setError(null);setPreview(null);
  ...
  try{const response=await fetch(`/api/source-blocks?${params}`,{method:'GET',cache:'no-store',signal:abort.signal}),body=await response.json();if(!response.ok||!body.preview){if(!abort.signal.aborted)setError(body.code==='source_history_incomplete'?'history':'provider');return}if(!abort.signal.aborted)setPreview(body.preview)}
  catch{if(!abort.signal.aborted)setError('provider')}
 ...
 return <section className="sourceBlockHistory" aria-label={en?'Provider preview':'Provider-Vorschau'} data-no-translate>
  ...
  {error&&<p className="sourceBlockError" role="alert">{error==='history'?(en?'The source history is incomplete. Complete the history import before checking this source. Nothing was changed.':'Die Source-Historie ist unvollständig. Erst den History-Import abschließen, dann diese Quelle erneut prüfen. Es wurde nichts geändert.'):(en?'The provider state could not be fully checked. Nothing was changed. Retry before confirming.':'Der Providerzustand konnte nicht vollständig geprüft werden. Es wurde nichts geändert. Vor einer Bestätigung erneut prüfen.')}</p>}
