...
export default function AffiliatePartnerPicker({partners,currentId,rangeParams}:{partners:AffiliatePickerPartner[];currentId?:string;rangeParams:string}){
 const router=useRouter(),rootRef=useRef<HTMLDivElement>(null),searchRef=useRef<HTMLInputElement>(null),[open,setOpen]=useState(false),[query,setQuery]=useState(''),[trafficFilter,setTrafficFilter]=useState<AffiliateTrafficFilter>('all'),[pins,setPins]=useState<string[]>([]);
 ...
 const current=partners.find(partner=>partner.id===currentId),visible=useMemo(()=>filterAffiliateChoices(sortAffiliateChoices(partners,pins,query),trafficFilter),[partners,pins,query,trafficFilter]),pinned=visible.filter(partner=>pins.includes(partner.id)),others=visible.filter(partner=>!pins.includes(partner.id));
 ...
 const select=(partner:AffiliatePickerPartner)=>{const params=new URLSearchParams(rangeParams);params.set('affiliate',partner.id);params.set('mode',trafficFilter==='all'?(partner.hasDirect?'direct':'smartlinks'):trafficFilter);setOpen(false);setQuery('');router.push(`/affiliates?${params.toString()}`)};
 ...
 return <div className="affiliatePicker" ref={rootRef}>
  ...
  <button type="button" className="affiliatePickerTrigger" aria-expanded={open} aria-controls="affiliate-picker-menu" onClick={()=>setOpen(value=>!value)}>
   <span className="affiliatePickerAvatar">{current?.name.slice(0,1).toUpperCase()||'A'}</span><span><small>{current?`Affiliate #${current.id}`:`${partners.length} Partner verfügbar`}</small><strong>{current?.name||'Affiliate auswählen'}</strong></span>{current&&pins.includes(current.id)&&<PinIcon active/>}<i aria-hidden="true">⌄</i>
  </button>
