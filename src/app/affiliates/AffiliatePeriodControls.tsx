...
export default function AffiliatePeriodControls({period,compact=false}:{period:ResolvedAffiliatePeriod;compact?:boolean}){
 ...
 if(compact)return <div className="affiliatePeriodControls compactPeriodControls" aria-busy={pending}><small>ZEITRAUM</small><div className="affiliatePresetButtons" role="group" aria-label="Schnelle Zeiträume">{presets.map(([id,label])=><button type="button" key={id} className={period.period===id?'active':''} aria-pressed={period.period===id} disabled={pending} onClick={()=>choosePreset(id)}>{label}</button>)}</div><b>{period.label}</b></div>;
