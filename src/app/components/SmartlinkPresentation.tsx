...
export function LandingpageSourceBreakdown({rows,scope,totalSois,landingpageId,coverage,onOpenChange,affiliateId='',affiliateName='Affiliate',offerId='',offerName='Offer',campaignId,canManage=false,embedded=false}:{rows:SmartlinkSourceBreakdown[];scope:string;totalSois:number;landingpageId:string;coverage?:SmartlinkSourceCoverage;onOpenChange?:(open:boolean)=>void;affiliateId?:string;affiliateName?:string;offerId?:string;offerName?:string;campaignId?:string;canManage?:boolean;embedded?:boolean}){
 ...
 const panel=<div className="lpSourcePanel">
  ...
  <section className="lpSourceSummary" aria-label="Zusammenfassung Quellenanalyse"><span><b>{num(rows.length)}</b><small>Quellen</small></span><span><b>{num(sourceSois)}</b><small>SOIs</small></span><span><b>{num(sourceFirstSales)}</b><small>First-Sales</small></span><span><b>{num(sourceRebills)}</b><small>Rebills</small></span><span><b>{num(sourceCoinSpend)}</b><small>Coin-Spend</small></span><span><b>{euro(sourceRevenue)}</b><small>Umsatz</small></span><span className={sourceProfit>=0?'up':'down'}><b>{euro(sourceProfit)}</b><small>Profit</small></span></section>
 ...
 return <section className="campaignSourceWorkspace"><header><div><span>CAMPAIGN-WEITER LP-VERGLEICH · EIGENES SOURCE-FENSTER</span><h3>Source × Landingpage</h3><p>Nur belastbare Source-Snapshots der aktiven Landingpages. Diese Summe erklärt nicht automatisch die Campaign-Bilanz im ausgewählten Zeitraum.</p></div><small>{scope}</small></header>
  <div className="campaignSourceMasterDetail"><nav className="campaignSourceList" aria-label="Sources über Landingpages">{rows.map(row=>{const identity=identityFor(row);return <button type="button" key={row.key} className={`${row.fit}${row.key===selected.key?' selected':''}`} aria-pressed={row.key===selected.key} onClick={()=>onSelect(row.key)}><span><b>{identity.primary}</b><small>{identity.missingMain||`${row.subLabel} · ${row.subSource||'Nicht übermittelt'}`} · {row.affectedLandingpages} Landingpage{row.affectedLandingpages===1?'':'s'}</small></span><span><b>{num(row.totals.sois)} SOIs</b><small>{num(row.totals.firstSales)} First-Sales · {num(row.totals.rebills)} Rebills</small></span><strong className={row.totals.profit>=0?'up':'down'}>{euro(row.totals.profit)}</strong></button>})}</nav>
   <article className={`campaignSourceComparison ${selected.fit}`} aria-live="polite"><header><div><span>{selectedIdentity.primaryLabel}{selectedIdentity.secondaryLabel?` / ${selectedIdentity.secondaryLabel}`:''}</span><h4>{selectedIdentity.primary}{selectedIdentity.secondary?` / ${selectedIdentity.secondary}`:''}</h4><small>{selectedIdentity.missingMain&&`${selectedIdentity.missingMain} · `}{selected.affectedLandingpages} Landingpage{selected.affectedLandingpages===1?'':'s'} · {selected.observation}</small></div><strong className={selected.totals.profit>=0?'up':'down'}>{euro(selected.totals.profit)}<small>Profit gesamt</small></strong></header>
    ...
    <div className="sourceLandingpageCells">{[...selected.cells].sort((a,b)=>(b.metrics?.profit??Number.NEGATIVE_INFINITY)-(a.metrics?.profit??Number.NEGATIVE_INFINITY)).map(cell=><section className={cell.state} key={cell.landingpageId}><header><div><span>LP #{cell.landingpageId} · Offer #{cell.offerId}</span><b>{cell.landingpageName}</b><small>{cell.weight} % Gewicht · {cell.status}</small></div>{cell.metrics?<strong className={cell.metrics.profit>=0?'up':'down'}>{euro(cell.metrics.profit)}</strong>:<strong>Unbekannt</strong>}</header>{cell.metrics?<div data-scope={`campaign-source-lp-events-${cell.landingpageId}`}><span><small>SOIs</small><b>{num(cell.metrics.sois)}</b></span><span><small>First-Sales</small><b>{num(cell.metrics.firstSales)} · {cell.metrics.firstSaleRate===null?'n/a':pct(cell.metrics.firstSaleRate)}</b></span><span><small>Rebills</small><b>{num(cell.metrics.rebills)}</b></span><span><small>Coin-Spend</small><b>{num(cell.metrics.coinSpend)}</b></span><span><small>Umsatz</small><b>{euro(cell.metrics.revenue)}</b></span><span><small>Payout</small><b>{euro(cell.metrics.payout)}</b></span><span><small>Profit</small><b className={cell.metrics.profit>=0?'up':'down'}>{euro(cell.metrics.profit)}</b></span></div>:<p>Source-Zelle unbekannt · Snapshot-Abdeckung dieser LP ist unvollständig oder nicht abgestimmt.</p>}</section>)}</div>
