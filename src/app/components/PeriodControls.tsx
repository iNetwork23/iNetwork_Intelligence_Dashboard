...
export default function PeriodControls({dimension,period,from,to,rangeLabel,maxDate,compact=false,error=null,todayNote=null,presets}:PeriodControlsProps){
 ...
 if(compact)return <div className={`periodControls compactPeriodControls${pending?' pending':''}`} aria-busy={pending}><small>ZEITRAUM</small>{presetButtons}{todayMarker}<b>{rangeLabel}</b>{feedback}</div>;
