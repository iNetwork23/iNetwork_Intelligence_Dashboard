...
export default function PeriodControls({dimension,period,from,to,rangeLabel,maxDate,compact=false,error=null,todayNote=null,presets}:PeriodControlsProps){
 ...
 if(compact)return localizeClientRoot(<div className={`periodControls compactPeriodControls${pending?' pending':''}`} aria-busy={pending}><small>ZEITRAUM</small>{presetButtons}{todayMarker}<b>{rangeLabel}</b>{feedback}</div>,locale);
 ...
 if(dimension==='source')return localizeClientRoot(<div ref={root} className="periodControls sourcePeriodControls" aria-busy={pending}>{presetButtons}{todayMarker}{monthPicker}{customForm}{feedback}</div>,locale);
 return localizeClientRoot(<section className="periodControls dashboardPeriod" aria-busy={pending}>
