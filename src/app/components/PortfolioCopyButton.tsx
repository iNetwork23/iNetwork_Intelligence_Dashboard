...
export default function PortfolioCopyButton({text,label}:{text:string;label:string}){
 ...
 return <LocalizedRoot><span className="portfolioCopyControl"><button type="button" className={`portfolioCopyButton ${state}`} onClick={copy} disabled={!text}><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="9" y="9" width="10" height="10" rx="2"/><path d="M15 9V7a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2"/></svg>{state==='copied'?'Kopiert':label}</button><span className="portfolioCopyFeedback" aria-live="polite">{state==='copied'?'Übersicht kopiert':state==='failed'?'Kopieren fehlgeschlagen':''}</span></span></LocalizedRoot>;
}
