...
import {DealRegisterConflictError,DealRegisterValidationError,readEditableDealRegister,saveDealRegister} from '@/lib/deal-register-store';
import {requirePermission} from '@/lib/session';
import {can} from '@/lib/rbac';
import {assertFreshWriteAccess,WriteAuthorizationError} from '@/lib/write-authorization';
...
export async function GET(){const auth=await authorize();if(!auth.ok)return auth.response;try{const state=await readEditableDealRegister();return json({rules:state.rules,defaults:DEFAULT_DEAL_RULES,source:state.source,revision:state.revision})}catch(error){console.error('Deal register could not be loaded',error);return json({error:'Deal-Register konnte nicht geladen werden'},503)}}
export async function PUT(request:Request){const auth=await authorize();if(!auth.ok)return auth.response;let origin:string;try{origin=canonicalOrigin(process.env.APP_ORIGIN,process.env.NODE_ENV,request.url)}catch{return json({error:'Serverkonfiguration ungültig'},500)}if(!checkCsrf(request,origin))return json({error:'Anfrage abgelehnt'},403);let input:Record<string,unknown>;try{input=await parseBoundedJson(request,131_072)}catch{return json({error:'Ungültige Anfrage (max. 128 KB)'},400)}
 const evidence=requestEvidence(request),actorId=auth.user.actorId;
 try{const{before,after,revision}=await saveDealRegister(input.rules,actorId,input.expectedRevision,securityStore(),new Date(),()=>assertFreshWriteAccess(auth.user,access=>access.role!=='partner'&&can(access,'settings.manage')));let audited=true;try{await audit({actorId,action:'deal_register.update',targetId:'deal_register:v1',before:{source:before.source,rules:before.rules},after:{source:'stored',rules:after,revision},...evidence})}catch(error){audited=false;console.error('Deal register audit failed',error)}return json({ok:true,rules:after,defaults:DEFAULT_DEAL_RULES,source:'stored',revision,audited})}
 catch(error){if(error instanceof WriteAuthorizationError)return json({error:error.message},403);if(error instanceof DealRegisterConflictError)return json({error:error.message},409);if(error instanceof DealRegisterValidationError)return json({error:error.message},400);console.error('Deal register update failed',error);await audit({actorId,action:'deal_register.update_failed',targetId:'deal_register:v1',after:{error:error instanceof Error?error.message:'unbekannt'},...evidence}).catch(()=>{});return json({error:'Deal-Register konnte nicht gespeichert werden'},503)}}
