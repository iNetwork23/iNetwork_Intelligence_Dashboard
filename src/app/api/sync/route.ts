...
export async function GET(request:NextRequest){
  ...
  try{
    const release=await acquireHistorySyncLock();try{const source=createEverflowHistorySource(process.env.EVERFLOW_API_KEY||'');const result=await runHistorySync({store:createSupabaseSyncStore(),loadConversions:source.loadConversions,loadReports:source.loadReports});if(result.conversionRows.length){await publishRebillDaySnapshots(result.conversionRows,{from:result.from,to:result.to});for(const affiliateId of new Set(result.conversionRows.map(row=>row.affiliate_id).filter(Boolean)))revalidateTag(`affiliate-rebills-${affiliateId}`,{expire:0})}const{conversionRows,...publicResult}=result;void conversionRows;expireSourceCaches();const campaigns=await syncCampaignSnapshots(process.env.EVERFLOW_API_KEY||'',12);return NextResponse.json({...publicResult,campaigns})}finally{try{revalidateTag('affiliate-rebills',{expire:0})}finally{await release()}}
