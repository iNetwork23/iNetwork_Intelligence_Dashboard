...
import{loadAffiliateConversionsFromCache}from'@/lib/cached-evaluations';
import type{ConversionRow}from'@/lib/everflow';
...
async function publishSourceCandidateRanges(started:number){
 ...
 // Conversions je Partner nur einmal je Lauf laden (beide Zeiträume nutzen dieselben 90 Tage); Reife-Kurzfassung nur im ersten Zeitraum persistieren.
 const conversionsMemo=new Map<string,Promise<ConversionRow[]>>(),conversionsFor=(affiliateId:string,now:Date)=>{let pending=conversionsMemo.get(affiliateId);if(!pending){pending=loadAffiliateConversionsFromCache(affiliateId,90,now);conversionsMemo.set(affiliateId,pending);pending.catch(()=>conversionsMemo.delete(affiliateId))}return pending};
