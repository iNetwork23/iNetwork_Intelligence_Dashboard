...
export async function GET(request:NextRequest){
 ...
 const scoped=filterPartnerRows(data,auth.user.access),rows=stripFinance(scoped,can(auth.user.access,'finance.view')),headers=[...new Set(rows.flatMap(row=>Object.keys(row)))];const csv=[...(truncated?[exportTruncationNotice(cappedAt,granularity)]:[]),headers.map(safeCell).join(','),...rows.map(row=>headers.map(key=>safeCell(row[key])).join(','))].join('\r\n');await audit({actorId:auth.user.actorId,action:'export.download',targetId:`${granularity==='month'?'daily_metrics_month':'daily_metrics'}:${rows.length}`,after:{rows:rows.length,from,to,finance:can(auth.user.access,'finance.view'),granularity,truncated},...requestEvidence(request)});return new NextResponse(csv,{headers:{...securityHeaders,'Content-Type':'text/csv; charset=utf-8','Content-Disposition':`attachment; filename="wlx-export${granularity==='month'?'-monat':''}.csv"`,...(truncated?{'X-Export-Truncated':'1'}:{})}});
}
