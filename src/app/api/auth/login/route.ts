...
import {canonicalOrigin,checkCsrf,createOpaqueSession,COOKIE_NAME,recordRateLimitFailure,resetRateLimit,securityHeaders} from '@/lib/security';
import {parseAccessMetadata} from '@/lib/rbac';
import {resolveLoginIdentity,usernameIdentityMatches} from '@/lib/user-provisioning';
import {consumeLoginRateLimits} from '@/lib/login-rate-limit';
...
export async function POST(request:Request){
 ...
 const form=new URLSearchParams(text),identifier=String(form.get('email')||form.get('username')||'').trim().slice(0,254),password=String(form.get('password')||'').slice(0,256),store=securityStore();const evidence=requestEvidence(request),now=Math.floor(Date.now()/1000),rate=await consumeLoginRateLimits(store,evidence.ip,identifier,now),{ipKey,identityKey,combinedKey}=rate;if(!rate.allowed){await audit({actorId:'anonymous',action:'login.rate_limited',targetId:identifier,...evidence});return failure(origin,429,rate.retryAfter)}
 let userId='',metadataVersion=0,actorEmail=identifier;
 const resolvedIdentity=await resolveLoginIdentity(store,identifier),authEmail=resolvedIdentity?.email||'invalid-login@invalid.local';
 try{const {data,error}=await getSupabasePasswordAuth().auth.signInWithPassword({email:authEmail,password});if(!error&&data.user&&(!resolvedIdentity?.index||usernameIdentityMatches(resolvedIdentity.index,data.user))){const access=parseAccessMetadata(data.user.app_metadata);if(access.status==='active'){userId=data.user.id;metadataVersion=access.version;actorEmail=data.user.email||authEmail;}}}catch{/* generic failure; optional legacy auth is evaluated below */}
 ...
 if(!userId){await Promise.all([ipKey,identityKey,combinedKey].map(key=>recordRateLimitFailure(store,key,now)));await audit({actorId:'anonymous',action:'login.failed',targetId:identifier,...evidence});return failure(origin)}
 await resetRateLimit(store,identityKey);await resetRateLimit(store,combinedKey);const {token}=await createOpaqueSession(store,{userId,metadataVersion});(await cookies()).set(COOKIE_NAME,token,{httpOnly:true,secure:true,sameSite:'strict',path:'/',maxAge:43_200});await audit({actorId:userId,action:'login.succeeded',targetId:actorEmail,...evidence});return NextResponse.redirect(new URL('/',origin),303);
}
