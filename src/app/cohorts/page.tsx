...
export default async function CohortsPage({searchParams}:{searchParams:Promise<{source?:string;sub_source?:string;affiliate?:string;page?:string;period?:string;from?:string;to?:string}>}){
  ...
  const hasFilters=[filters.source,filters.sub_source,filters.affiliate].some(value=>Boolean(value?.trim()));
  ...
  {!result.total&&<section className="smartEmpty"><h2>{hasFilters?'Keine Kohorten für diese Filter':'Noch keine Kohorten vorhanden'}</h2><p>{hasFilters?'Filter ändern oder zurücksetzen, um andere Kohorten anzuzeigen.':'Nach den ersten erfolgreichen Sync-Chunks erscheinen die Daten hier.'}</p></section>}</main>;
}
