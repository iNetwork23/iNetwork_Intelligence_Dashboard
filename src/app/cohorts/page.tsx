...
import {getCohortClient} from '@/lib/supabase';
...
export default async function CohortsPage({searchParams}:{searchParams:Promise<{source?:string;sub_source?:string}>}){
  ...
  let rows;try{rows=await getLtvCohorts(getCohortClient(),{source:filters.source,subSource:filters.sub_source})}catch(error){console.error(error);return <main className="fatal"><h1>Kohorten konnten nicht geladen werden</h1><p>Migration und Supabase-Umgebungsvariablen prüfen.</p></main>}
  return <main className="dashboard"><header className="topbar"><div><div className="eyebrow">ME MEDIA · LTV</div><h1>Umsatzkohorten</h1><p>Registrierungsmonat × kumulierter Umsatz nach Lead-Alter.</p></div><div className="topActions"><InstantLink className="logout" href="/">Account Monitor</InstantLink><InstantLink className="logout active" href="/cohorts" aria-current="page">LTV-Kohorten</InstantLink><InstantLink className="logout" href="/affiliates">Affiliate Optimizer</InstantLink><InstantLink className="logout" href="/automation">Auto-Rotation</InstantLink><InstantLink className="logout" href="/smartlinks">Smartlink Intelligence</InstantLink><div className="live"><span/>Read only</div><form action="/api/auth/logout" method="post"><button className="logout">Abmelden</button></form></div></header>
