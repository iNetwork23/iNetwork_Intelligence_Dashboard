...
export async function getAffiliateSourceFreshness(range:{from:string;to:string}){if(!range.from||!range.to)throw new Error('Auswertungszeitraum fehlt');return unstable_cache(()=>loadSourceSnapshotFreshness(range),['affiliate-source-freshness-v1',range.from,range.to],{revalidate:300,tags:['affiliate-source-freshness']})()}
