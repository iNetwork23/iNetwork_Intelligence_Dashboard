...
vi.mock('./supabase',()=>({getSupabaseAdmin:()=>({from:()=>({select:()=>({gte:()=>({lt:()=>summaryRows()})})})})}));
...
const summary=(affiliateId:string,youngByUrl:Record<string,number>,confidence:LeadYoungSummary['confidence']='hoch'):LeadYoungSummary=>({version:1,affiliateId,generatedAt:'2026-09-04T11:47:00.000Z',p75Hours:30,confidence,fallbackUsed:false,youngByUrl});
