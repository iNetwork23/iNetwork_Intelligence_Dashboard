...
describe('smartlink intelligence loader',()=>{
 ...
 it('loads accountwide Campaign-Affiliate assignments without pretending they are direct traffic',async()=>{const fetcher=vi.fn().mockImplementation(()=>response({table:[]}));await loadCampaignAffiliateRows('secret',fetcher,new Date('2026-07-22T12:00:00Z'));const body=JSON.parse(fetcher.mock.calls[0][1].body);expect(body.columns.map((x:{column:string})=>x.column)).toEqual(['campaign','affiliate']);expect(body.query.filters).toEqual([])});
 it('batches one affiliate Smartlink report and keeps Campaign rows scoped',async()=>{const campaign={network_campaign_id:2,campaign_name:'TrafficPartner',campaign_status:'active',redirect_routing_type:'weight',relationship:{redirects:{entries:[]}}},row={columns:[{column_type:'campaign',id:'2',label:'TrafficPartner'},{column_type:'affiliate',id:'6',label:'TrafficPartner'},{column_type:'offer',id:'57',label:'Singles69'},{column_type:'offer_url',id:'10',label:'LP'},{column_type:'hour',id:'1784707200',label:'1784707200'}],reporting:{total_click:10,cv:1,payout:3,revenue:5,profit:2}},fetcher=vi.fn().mockImplementation((url:string,init?:RequestInit)=>url.includes('/campaigns/2')?response(campaign):response({table:JSON.parse(String(init?.body)).columns.some((x:{column:string})=>x.column==='event_name')?[]:[row]}));const result=await loadAffiliateSmartlinkInsights('6',[2],'secret',fetcher,new Date('2026-07-22T12:00:00Z'));expect(result[0].identity).toMatchObject({campaignId:2,affiliateId:6});const reports=fetcher.mock.calls.filter(call=>String(call[0]).includes('/reporting/entity/table'));expect(reports).toHaveLength(2);for(const call of reports)expect(JSON.parse(call[1].body).query.filters).toEqual([{resource_type:'affiliate',filter_id_value:'6'}])});
});
