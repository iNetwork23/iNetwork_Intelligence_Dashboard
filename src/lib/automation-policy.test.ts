...
import {automationScopeAllowed,filterAutomationOffersByScope,mayConfigureAutomation,mayRunLiveAutomation} from './automation-policy';
import {parseAccessMetadata} from './rbac';
describe('automation access policy',()=>{
 it('requires explicit automation permission and excludes partners',()=>{expect(mayConfigureAutomation(parseAccessMetadata({role:'partner',grants:['automations.manage']}))).toBe(false);expect(mayConfigureAutomation(parseAccessMetadata({role:'employee',grants:['automations.manage','campaigns.edit']}))).toBe(true);expect(mayRunLiveAutomation(parseAccessMetadata({role:'employee',grants:['automations.manage','campaigns.edit']}))).toBe(false);expect(mayRunLiveAutomation(parseAccessMetadata({role:'super_admin'}))).toBe(true)});
