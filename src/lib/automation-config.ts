...
export function normalizeAutomationDraft(raw:unknown,now=new Date()):AutomationConfiguration{
 ...
 const offers:AutomationOffer[]=offersRaw.slice(0,20).map(item=>{const offer=object(item),landingpagesRaw=Array.isArray(offer.landingpages)?offer.landingpages:[];return{
  offerId:positiveInt(offer.offerId,0),offerName:text(offer.offerName,`Offer #${positiveInt(offer.offerId,0)}`),landingpages:landingpagesRaw.slice(0,100).map(value=>{const lp=object(value);return{familyKey:text(lp.familyKey).toLowerCase(),familyName:text(lp.familyName),offerUrlId:positiveInt(lp.offerUrlId,0),offerUrlName:text(lp.offerUrlName),status:text(lp.status,'unknown').toLowerCase(),selection:strictEnum(lp.selection,['active','candidate','excluded'] as const,'active','Landingpage-Auswahlstatus')}}),
 }});
 ...
 const weights=equalWeights(candidates.length),date=now.toISOString(),schedule=object(input.schedule),thresholds=object(input.thresholds),weightInput=object(input.weights),weightMode=strictEnum(weightInput.mode,['equal','champion_challenger'] as const,'equal','Gewichtungsmodus'),championOfferUrlId=positiveInt(weightInput.championOfferUrlId,0),testMode=strictEnum(input.testMode,['single_offer','multi_offer'] as const,'single_offer','Testtyp'),strategy=strategyFor(input.strategy,testMode);
 return{
  ...
  testMode,strategy,objective:strictEnum(input.objective,['sale_first','profit_per_soi','profit_epc'] as const,'sale_first','Zielmetrik'),offers,
...
export function validateAutomationDraft(config:AutomationConfiguration){const errors:string[]=[];
 if(!config.name)errors.push('Name fehlt.');if(config.name.length>120)errors.push('Name ist zu lang.');
 if(config.affiliateId<=0)errors.push('Affiliate-ID fehlt.');if(config.campaignId<=0)errors.push('Campaign-ID fehlt.');
 const validTestMode=['single_offer','multi_offer'].includes(config.testMode as string),validStrategy=['equal_slots','champion_challenger','matched_rounds'].includes(config.strategy as string);
 if(!validTestMode)errors.push('Testtyp wird nicht unterstützt.');if(!validStrategy)errors.push('Automationsstrategie wird nicht unterstützt.');
 if(!['sale_first','profit_per_soi','profit_epc'].includes(config.objective as string))errors.push('Zielmetrik wird nicht unterstützt.');
 if(!['equal','champion_challenger'].includes(config.weights.mode as string))errors.push('Gewichtungsmodus wird nicht unterstützt.');
 ...
 if(validTestMode&&(!validStrategy||(config.testMode==='single_offer'&&!['equal_slots','champion_challenger'].includes(config.strategy as string))||(config.testMode==='multi_offer'&&config.strategy!=='matched_rounds')))errors.push('Strategie passt nicht zum Testtyp.');
