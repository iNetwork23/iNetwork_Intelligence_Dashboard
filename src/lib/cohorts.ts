...
export async function getLtvCohorts(filters:LtvFilters,access:AccessMetadata){
 // Empty GET form controls mean "all", not the SQL dimension value ''.
 // Preserve nonblank identifiers exactly; authorization scopes remain separate.
 const source=filters.source?.trim()?filters.source:undefined,subSource=filters.subSource?.trim()?filters.subSource:undefined;
 ...
 if(foreignScopeRequested(access,{source,sub_source:subSource}))throw new Error('403 · Fremder Datenscope');
 ...
 if(isScopeRestricted(access)){
  result=await supabase.rpc('ltv_cohorts_scoped_v1',{
   ...
   p_sub_source:subSource??null,
  });
 }else{
  result=await supabase.rpc('ltv_cohorts_internal_v1',{p_source:source??null,p_sub_source:subSource??null});
 }
