...
export async function getAffiliateSmartlinks(affiliateId:string,campaignIds:number[],range?:{from:string;to:string}){const ids=Array.from(new Set(campaignIds)).sort((a,b)=>a-b),selected=range||{from:'',to:''};return unstable_cache(()=>loadAffiliateSmartlinkInsightsFromCache(affiliateId,ids,new Date(),range),['affiliate-smartlinks-cache-v3',affiliateId,ids.join(','),selected.from,selected.to,day()],{revalidate:300,tags:[`affiliate-smartlinks-${affiliateId}`]})()}
