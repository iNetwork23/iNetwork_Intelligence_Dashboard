...
import {loadAffiliateSmartlinkInsights,loadCampaignAffiliateRows,loadSmartlinkInsight,searchCampaigns} from './everflow';
...
const campaignAffiliateRows=unstable_cache(()=>loadCampaignAffiliateRows(key()),['campaign-affiliate-directory',day()],{revalidate:300,tags:['campaign-affiliate-directory']});
...
export async function getAffiliateSmartlinks(affiliateId:string,campaignIds:number[]){const ids=[...new Set(campaignIds)].sort((a,b)=>a-b);return unstable_cache(()=>loadAffiliateSmartlinkInsights(affiliateId,ids,key()),['affiliate-smartlinks',affiliateId,ids.join(','),day()],{revalidate:60,tags:[`affiliate-smartlinks-${affiliateId}`]})()}
