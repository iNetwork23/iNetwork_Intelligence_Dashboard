...
export function getSmartlinkInsight(campaignId:number,access:AccessMetadata,bypass=false,requestedAffiliateId?:string){
 ...
 const insight=bypass?load():unstable_cache(load,['smartlink-intelligence-cache-v7-berlin-v5',String(campaignId),affiliateId||'unscoped',fingerprint,day()],{revalidate:300,tags:[`smartlink-${campaignId}`]})();
...
export async function getAffiliateSmartlinks(affiliateId:string,campaignIds:number[],range:{from:string;to:string}|undefined,access:AccessMetadata,bypass=false,includeSources=true){
 ...
 const insights=await(bypass?load():unstable_cache(load,['affiliate-smartlinks-cache-v9-berlin-v5',includeSources?'full':'compact',affiliateId,ids.join(','),selected.from,selected.to,fingerprint,day()],{revalidate:300,tags:[`affiliate-smartlinks-${affiliateId}`]})()),deals=await loadDealRegister();
