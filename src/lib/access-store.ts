...
export class SyncStateSecurityStore implements SecurityStore{
 ...
 /** Atomarer Vergleich im UPDATE: zwei Bearbeiter derselben Revision können nicht beide speichern. */
 async replaceIfRevision(key:string,revision:string|null,value:unknown){const query=getSupabaseAdmin().from('sync_state').update({value}).eq('key',key),guarded=revision===null?query.is('value->>revision',null):query.eq('value->>revision',revision);const {data,error}=await guarded.select('key').maybeSingle();if(error)throw new Error('Sicherheitsstatus nicht verfügbar');return Boolean(data)}
