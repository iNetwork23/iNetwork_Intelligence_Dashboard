...
const fixture=vi.hoisted(()=>({checkpoint:null as FraudBackfillState|null,cache:new Map<string,Promise<unknown>>(),reads:0,readError:false}));
...
vi.mock('./supabase',()=>({getSupabaseAdmin:()=>({from:()=>{
 ...
 range:async()=>({data:[],error:null}),then:(done:(value:unknown)=>unknown)=>Promise.resolve(done({data:[],error:null}))};return chain;
}})}));
...
beforeEach(()=>{fixture.cache.clear();fixture.reads=0;fixture.readError=false;fixture.checkpoint={...initialFraudBackfillState(new Date('2026-09-08T12:00Z')),phase:'rolling',coveredFrom:'2026-05-12',coveredThrough:'2026-09-08',parityVerifiedThrough:'2026-09-08',readyAt:'2026-09-08T11:00Z',lastSuccessAt:'2026-09-08T11:00Z'}});
...
it('does not serve a warm ready dashboard after a partial import invalidates its checkpoint',async()=>{
 ...
 expect(invalidated.coverage.cutoverReady).toBe(false);expect(invalidated.coverage.conversionJoin).toBeNull();expect(invalidated.stopCompliance).toEqual([]);expect(fixture.reads).toBe(3);expect(fixture.cache.size).toBe(2);
});
