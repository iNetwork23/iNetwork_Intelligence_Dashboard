...
export async function syncCampaignSnapshots(apiKey:string,limit=60){
  ...
  const detailMap=new Map(details.map(shape=>[shape.network_campaign_id,shape])),now=new Date().toISOString(),generation=newSnapshotGeneration(),rows=directory.map(item=>{const previous=existingMap.get(String(item.network_campaign_id)),fresh=detailMap.get(item.network_campaign_id);return{key:`${GENERATION_PREFIX}${generation}:${item.network_campaign_id}`,value:{campaign_id:String(item.network_campaign_id),campaign_name:item.campaign_name,campaign_status:item.campaign_status,payload:fresh||(previous?.payload&&validShape(previous.payload,item.network_campaign_id)?{...previous.payload,campaign_status:item.campaign_status}:{}),synced_at:fresh?now:(previous?.synced_at||now)} satisfies SnapshotValue}}),pending=rows.filter(row=>!validShape(row.value.payload,Number(row.value.campaign_id))).length;
