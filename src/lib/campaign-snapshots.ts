...
export type CampaignDirectoryItem={network_campaign_id:number;campaign_name:string;campaign_status:string};
...
export async function loadCampaignDirectoryFromCache():Promise<CampaignDirectoryItem[]>{const data=await loadAllSnapshotRows();if(!data.length)throw new Error('Campaign-Metadaten-Generation fehlt');const directory=data.map(row=>row.value).map(value=>({network_campaign_id:Number(value.campaign_id),campaign_name:value.campaign_name,campaign_status:value.campaign_status}));if(!validDirectory(directory))throw new Error('Campaign-Metadaten-Generation ist beschädigt');return directory.sort((a,b)=>a.network_campaign_id-b.network_campaign_id)}
...
export async function syncCampaignSnapshots(apiKey:string,limit=60){
  ...
  const directoryResponse=await everflowJson<{campaigns?:CampaignDirectoryItem[]}>(`${BASE}/networks/campaigns`,apiKey);if(!validDirectory(directoryResponse.campaigns))throw new Error('Campaign-Verzeichnis ist leer, doppelt oder unvollständig');const directory=directoryResponse.campaigns,supabase=getSupabaseAdmin(),existing=await loadAllSnapshotRows(),existingMap=new Map(existing.map(row=>[row.value.campaign_id,row.value])),staleBefore=Date.now()-24*60*60_000;
