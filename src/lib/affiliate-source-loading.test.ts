...
const state = vi.hoisted(() => ({batches: [] as string[][], signals: [] as AbortSignal[], active: 0, peak: 0, failDay: ''}));
...
vi.mock('./supabase', () => ({getSupabaseAdmin: () => ({from: () => {
  ...
  const query = {
    ...
    then: async (resolve: (value: unknown) => unknown) => {
      ...
      return resolve({data: keys.map(key => ({value: {date: key.split(':')[1], affiliate_id: '154', affiliate_name: 'Partner', rows: [
...
beforeEach(() => {state.batches = []; state.signals = []; state.active = 0; state.peak = 0; state.failDay = '';});
