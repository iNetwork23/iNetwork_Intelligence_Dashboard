...
describe('copy-ready portfolio reports',()=>{
 ...
 it('wires total, company, and brand/offer copy actions into the account dashboard',()=>{const page=readFileSync(join(process.cwd(),'src/app/page.tsx'),'utf8'),css=readFileSync(join(process.cwd(),'src/app/globals.css'),'utf8');for(const marker of ['Gesamtübersicht kopieren','Firma kopieren','Brand / Offer kopieren','Firmen / Affiliates','Brands / Offers'])expect(page).toContain(marker);expect(page).toContain("buildPortfolioCopyReport");expect(page).toContain("scope:'affiliate'");expect(page).toContain("scope:'offer'");for(const marker of ['.portfolioCopyBar','.portfolioCopyButton','.portfolioCopyControl'])expect(css).toContain(marker)});
});
