...
/** Isolated password-auth client: never mutates the privileged singleton's auth state. */
export function getSupabasePasswordAuth(){
...
async function pruneDaySnapshotRange(from:string,to:string,activeGenerations:ReadonlyMap<string,string>){
  const supabase=getSupabaseAdmin(),cutoff=Date.now()-24*60*60_000,namespaces=['source_day:','campaign_affiliate_day:','portfolio_day:'];
  for(const prefix of namespaces){
    const keys:string[]=[];
    for(let start=0;;start+=1000){
      const{data,error}=await supabase.from('sync_state').select('key').gte('key',`${prefix}${from}:`).lt('key',`${prefix}${nextDay(to)}:`).order('key').range(start,start+999);
      throwIfError(error,'snapshot generation range list');
      const batch=(data||[]).map(row=>row.key as string);keys.push(...batch);if(batch.length<1000)break;
    }
    const stale=keys.filter(key=>{const remainder=key.slice(prefix.length),separator=remainder.indexOf(':'),day=remainder.slice(0,separator),generation=remainder.slice(separator+1).split(':')[0],created=snapshotGenerationCreatedAt(generation);return generation!==activeGenerations.get(day)&&created!==null&&created<cutoff});
    for(let start=0;start<stale.length;start+=200){const{error}=await supabase.from('sync_state').delete().in('key',stale.slice(start,start+200));throwIfError(error,'stale snapshot generation range delete')}
  }
}
...
async function upsertSourceSnapshots(from:string,to:string,rows:DailyMetricRow[]){
  ...
  for(let day=from;day<=to;day=nextDay(day)){
    const generation=newSnapshotGeneration(),affiliates=byDay.get(day)||new Map<string,DailyMetricRow[]>(),dayRows=Array.from(affiliates.values()).flat(),snapshots:{key:string;value:unknown}[]=Array.from(affiliates,([affiliateId,metrics])=>({key:`source_day:${day}:${generation}:${affiliateId}`,value:{version:5,timezoneId:56,date:day,affiliate_id:affiliateId,affiliate_name:metrics[0].affiliate_name,rows:metrics.map(encodeSourceSnapshotRow)}}));
    ...
    const marker={version:5,timezoneId:56,date:day,generation};
  ...
  await pruneDaySnapshotRange(from,to,activeGenerations);
}
...
async function acquireMetricReplaceLock(){return acquireSyncStateLock('daily_metrics_replace_lock',10*60_000,'Reporting-Metrikersatz')}
