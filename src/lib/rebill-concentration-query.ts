...
export type StoredRebillEvent={type:string;lead_id:string|null;converted_at:string;campaign_id:string|null;offer_id:string|null;offer_url_id:string|null;status:string|null};
const shift=(day:string,days:number)=>new Date(Date.parse(`${day}T00:00:00Z`)+days*86_400_000).toISOString();
export function rebillQueryEnvelope(range:{from:string;to:string}){return{from:shift(range.from,-1),toExclusive:shift(range.to,2)}}
export function mapStoredRebillEvent(row:StoredRebillEvent):RebillEvent|null{if((row.type!=='first_sale'&&row.type!=='rebill')||(row.status!==null&&row.status!=='approved'))return null;return{type:row.type,customerId:row.lead_id?.trim()||null,convertedAt:row.converted_at,campaignId:row.campaign_id||'0',offerId:row.offer_id||'0',offerUrlId:row.offer_url_id||'0'}}
