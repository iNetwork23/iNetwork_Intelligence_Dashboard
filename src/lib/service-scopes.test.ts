...
import {campaignDirectoryForAccess,partnerAffiliateForSmartlink,assertAffiliateOptimizerAggregateAccess,sourceRowsForAccess} from './service-scopes';
const campaign=(id:number)=>({network_campaign_id:id,campaign_name:`C${id}`,campaign_status:'active'});
const sourceRow=(affiliate:string,offer:string,source:string,subSource:string)=>({columns:[{column_type:'affiliate',id:affiliate},{column_type:'offer',id:offer},{column_type:'campaign',id:'0'},{column_type:'source_id',id:source},{column_type:'sub1',id:subSource}],reporting:{cv:1}});
