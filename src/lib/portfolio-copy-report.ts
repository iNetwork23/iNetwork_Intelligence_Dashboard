...
const summary=(value:Metrics)=>`${number(value.sois)} Leads (SOIs) · ${euro(value.payout)} Payout · ${euro(value.revenue)} Umsatz · ${euro(value.profit)} Profit · ${number(value.firstSales)} First-Sales · ${number(value.rebills)} Rebills`;
const line=(name:string,id:string,value:Metrics)=>`- ${name} (#${id}): ${number(value.sois)} Leads · ${euro(value.payout)} Payout · ${euro(value.revenue)} Umsatz · ${euro(value.profit)} Profit`;
function grouped(rows:PathRow[],dimension:'affiliate'|'offer'){
 ...
 return [...map.values()].filter(item=>item.value.sois||item.value.payout||item.value.revenue).sort((a,b)=>b.value.sois-a.value.sois||a.name.localeCompare(b.name,'de'));
}
export function buildPortfolioCopyReport(input:PortfolioCopyInput,scope:PortfolioCopyScope){
 const active=input.paths.filter(row=>row.clicks||row.sois||row.firstSales||row.rebills||row.payout||row.revenue);
 ...
 return ['Gesamtübersicht · '+input.rangeLabel,summary(input.totals),'','Nach Firma / Affiliate',...grouped(active,'affiliate').map(item=>line(item.name,item.id,item.value)),'','Nach Brand / Offer',...grouped(active,'offer').map(item=>line(item.name,item.id,item.value))].join('\n');
}
