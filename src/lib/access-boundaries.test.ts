...
describe('wired server authorization boundaries',()=>{
 ...
 it('preserves and authorizes the affiliate context for Smartlink source analysis',()=>{const page=read('smartlinks/page.tsx'),service=readFileSync(new URL('./smartlink-service.ts',import.meta.url),'utf8');expect(page).toContain("requestedPartner=/^\\d+$/.test(query.partner||'')?query.partner:undefined");expect(page).toContain("foreignScopeRequested(user.access,{campaign:query.campaign,affiliate:requestedPartner||query.affiliate})");expect(page).toContain("getSmartlinkInsight(campaignId,user.access,query.refresh==='1',query.affiliate)");expect(service).toContain("foreignScopeRequested(access,{campaign:String(campaignId),affiliate:requestedAffiliateId})");expect(service).toContain("scopedAffiliateId=partnerAffiliateForSmartlink(access),affiliateId=requestedAffiliateId||scopedAffiliateId");expect(service).toContain("String(campaignId),affiliateId||'unscoped',fingerprint");});
