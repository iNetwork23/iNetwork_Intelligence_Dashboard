...
describe('wired server authorization boundaries',()=>{
 ...
 it('preserves and authorizes the affiliate context for Smartlink source analysis',()=>{const page=read('affiliates/page.tsx'),legacy=read('smartlinks/page.tsx'),service=readFileSync(new URL('./smartlink-service.ts',import.meta.url),'utf8');expect(page).toContain('campaign: query.campaign');expect(page).toContain('getAffiliateSmartlinks(');expect(legacy).toContain('foreignScopeRequested(user.access,{campaign:query.campaign,affiliate:requestedAffiliate})');expect(legacy).toContain('legacySmartlinkRedirectHref');expect(service).toContain("foreignScopeRequested(access,{campaign:String(campaignId),affiliate:requestedAffiliateId})");expect(service).toContain("scopedAffiliateId=partnerAffiliateForSmartlink(access),affiliateId=requestedAffiliateId||scopedAffiliateId");expect(service).toContain("String(campaignId),affiliateId||'unscoped',fingerprint");});
it('keeps migrated Smartlink intelligence behind its former permissions',()=>{const page=read('affiliates/page.tsx');expect(page).toContain('query.mode === "smartlinks"');expect(page).toContain('can(user.access, "smartlinks.view")');expect(page).toContain('can(user.access, "finance.view")')});
 it('rechecks Smartlink source tuples server-side instead of trusting display labels',()=>{const route=read('api/source-blocks/route.ts'),model=readFileSync(new URL('./source-blocks.ts',import.meta.url),'utf8');expect(route).toContain('sourceBlockVisibleInSnapshotRows');expect(route).toContain('sourceBlockOffersFromSnapshotRows');expect(route).not.toContain('getAffiliateSmartlinks');expect(model).toContain('block.originCampaignId===null');expect(model).toContain("snapshotDimension(row,'campaign')?.id===String(block.originCampaignId)");expect(model).toContain("snapshotSourceValue(row,'source_id')!==block.mainValue");expect(model).toContain("snapshotSourceValue(row,'sub1')===block.subValue")});
