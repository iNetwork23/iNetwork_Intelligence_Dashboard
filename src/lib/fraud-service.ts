...
},['fraud-dashboard-v7-berlin-v5',range.from,range.to,accessFingerprint,JSON.stringify(filters),createHash('sha256').update(JSON.stringify(markers??null)).digest('hex'),createHash('sha256').update(JSON.stringify(backfill)).digest('hex')],{revalidate:300,tags:['fraud-dashboard','affiliate-source']})();

export async function getFraudDashboard(range:{from:string;to:string},access:AccessMetadata,filters:FraudDashboardFilters={},markers?:SourceBlockMarkerIndex){assertFraudAccess(access);assertFraudRange(range);const backfill=await loadFraudBackfillState();return dashboardCache(range,scopeFingerprint(access),filters,markers,backfill)}
