...
export function evaluateSourceCandidates(rows:SourceBreakdownRow[],labels:Labels,trends?:Map<string,SourceCandidateTrend>):SourceCandidate[]{
 ...
 for(const bucket of byUrl.values()){const first=bucket[0];if(first.trafficMode==='unknown')continue;const names=labels.paths.get(`${first.offerId}|${first.offerUrlId}`)||{offer:first.offerId,offerUrl:first.offerUrlId};
