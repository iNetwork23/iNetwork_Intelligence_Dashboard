import {classifyTrafficPath,fraudAttributionPath,normalizeFraudSource,type FraudConversionInput,type FraudMetricInput,type FraudTrafficMode} from './fraud-control';
...
export function fraudMetricFromReportRow(row:FraudReportRow):FraudMetricInput{
  ...
  const sourceInput={trafficMode,sourceId,sub1:dim(row,'sub1').id,sub2:dim(row,'sub2').id,sub3:dim(row,'sub3').id,sub4:dim(row,'sub4').id,sub5:dim(row,'sub5').id,adv1,adv2},source=normalizeFraudSource(sourceInput);
  return{date:dim(row,'date').id,affiliateId:dim(row,'affiliate').id,affiliateName:dim(row,'affiliate').label,offerId:offer.id,offerName:offer.label,campaignId:campaign.id||'0',campaignName:campaign.label||'Direct',offerUrlId:dim(row,'offer_url').id||'0',offerUrlName:dim(row,'offer_url').label||'N/A',trafficMode,...source,attributionPath:fraudAttributionPath(sourceInput),clicks:value(row,'total_click'),sois:value(row,'cv'),firstSales:value(row,'first_sales'),rebills:value(row,'rebills'),coinEvents:value(row,'coin_spend'),payout:value(row,'payout'),revenue:value(row,'revenue')};
}
...
export function fraudConversionFromCacheRecord(row:CacheRecord):FraudConversionInput{
  ...
  const raw=row.raw&&typeof row.raw==='object'?row.raw as Record<string,unknown>:{},text=(key:string)=>typeof raw[key]==='string'?raw[key] as string:'';
  const sourceInput={trafficMode,sourceId:text('source_id')||row.source_id,sub1:text('sub1'),sub2:text('sub2'),sub3:text('sub3'),sub4:text('sub4'),sub5:text('sub5'),adv1:text('adv1'),adv2:text('adv2')};
  return{id:row.id,type:row.type as FraudConversionInput['type'],convertedAt:row.converted_at,clickAt:row.click_at,affiliateId:row.affiliate_id||'0',affiliateName:row.affiliate_name||'N/A',offerId:row.offer_id||'0',offerName:row.offer_name||'N/A',campaignId:row.campaign_id||'0',campaignName:row.campaign_name||'Direct',offerUrlId:row.offer_url_id||'0',offerUrlName:row.offer_url_name||'N/A',trafficMode,source:row.source_id||'Nicht übermittelt',subSource:row.sub_source||'Nicht übermittelt',sourceDimension:(row.source_dimension||'unknown') as FraudConversionInput['sourceDimension'],subSourceDimension:(row.sub_source_dimension||'unknown') as FraudConversionInput['subSourceDimension'],attributionPath:fraudAttributionPath(sourceInput),leadId:row.lead_id,status:row.status,isScrub:Boolean(row.is_scrub),errorCode:row.error_code,payout:Number(row.payout||0),revenue:Number(row.revenue||0)};
}
