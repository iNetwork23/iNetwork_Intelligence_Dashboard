...
export type DailySourceRow={affiliate_id:string;affiliate_name:string;offer_id:string;offer_name:string;campaign_id:string;campaign_name:string;offer_url_id:string;offer_url_name:string;source_id:string;sub_source:string;clicks:number|string;sois:number|string;first_sales:number|string;rebills:number|string;coin_spend:number|string;payout:number|string;revenue:number|string;profit:number|string;raw?:{traffic_mode?:'api'|'tracked'|'unknown';source_dimension?:string;sub_source_dimension?:string;adv1?:string;adv2?:string}|null};
export type SourceSnapshotRow={o:string;on:string;c:string;cn:string;u:string;un:string;s:string;ss:string;m?:'api'|'tracked'|'unknown';sd?:string;ssd?:string;a1?:string;a2?:string;cl:number;cv:number;fs:number;rb:number;cs:number;p:number;r:number;pr:number};
...
export const encodeSourceSnapshotRow=(row:DailyMetricRow):SourceSnapshotRow=>({o:row.offer_id,on:row.offer_name,c:row.campaign_id,cn:row.campaign_name,u:row.offer_url_id,un:row.offer_url_name,s:row.source_id,ss:row.sub_source,m:row.raw.traffic_mode as'api'|'tracked'|'unknown'|undefined,sd:String(row.raw.source_dimension||''),ssd:String(row.raw.sub_source_dimension||''),a1:String(row.raw.adv1||''),a2:String(row.raw.adv2||''),cl:row.clicks,cv:row.sois,fs:row.first_sales,rb:row.rebills,cs:row.coin_spend,p:row.payout,r:row.revenue,pr:row.profit});
...
export function mapAffiliateSourceRows(rows:DailySourceRow[],metricDate?:string):ReportRow[]{
  const grouped=new Map<string,DailySourceRow&{traffic_mode:'api'|'tracked'|'unknown';source_value:string;sub_value:string}>();
  for(const row of rows){
    const inferred=classifyTrafficPath({campaignId:row.campaign_id,clicks:n(row.clicks),offerUrlId:row.offer_url_id,adv1:row.raw?.adv1,adv2:row.raw?.adv2}),traffic_mode=row.raw?.traffic_mode||(inferred==='clickless_api'?'api':inferred==='unknown'?'unknown':'tracked'),source_value=sourceText(traffic_mode==='api'?row.raw?.adv1:row.source_id)||'Nicht übermittelt',sub_value=sourceText(traffic_mode==='api'?row.raw?.adv2:row.sub_source),source_dimension=row.raw?.source_dimension||'unknown',sub_source_dimension=row.raw?.sub_source_dimension||'unknown';
    const key=JSON.stringify([row.affiliate_id,row.offer_id,row.campaign_id,row.offer_url_id,traffic_mode,source_dimension,source_value,sub_source_dimension,sub_value]);
  ...
  return Array.from(grouped.values()).map(row=>({columns:[
    ...
    {column_type:'sub1',id:row.sub_value||'N/A',label:row.sub_value||'N/A'},{column_type:'sub_source_dimension',id:row.raw?.sub_source_dimension||'unknown',label:row.raw?.sub_source_dimension||'unknown'},
