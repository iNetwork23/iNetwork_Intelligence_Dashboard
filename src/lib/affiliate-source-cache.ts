import {isBerlinDayMarker} from './berlin-reporting-contract';
...
export type PortfolioSnapshotRow=SourceSnapshotRow&{a:string;an:string;ce?:boolean};
export type SourceSnapshotGeneration={version:number;timezoneId?:number;date:string;generation:string};
export type SourceSnapshotCoverage={from:string;to:string;acceptedFrom:string|null;acceptedTo:string|null;acceptedDays:number;expectedDays:number;missingDays:string[]};
...
export const encodePortfolioSnapshotRow=(row:DailyMetricRow):PortfolioSnapshotRow=>({a:row.affiliate_id,an:row.affiliate_name,ce:row.raw.click_metrics_eligible===undefined?row.raw.traffic_mode==='tracked':row.raw.click_metrics_eligible===true,...encodeSourceSnapshotRow(row)});
...
export function availableSourceSnapshotDays(range:{from:string;to:string},markers:SourceSnapshotGeneration[],options:{minimumVersion?:number}={}){const minimumVersion=options.minimumVersion??2;return markers.filter(marker=>marker.version>=minimumVersion&&(minimumVersion<5||isBerlinDayMarker(marker))&&marker.date>=range.from&&marker.date<=range.to&&Boolean(marker.generation)).sort((a,b)=>a.date.localeCompare(b.date))}
const nextDay=(day:string)=>new Date(Date.parse(`${day}T12:00:00Z`)+86_400_000).toISOString().slice(0,10);
export function resolveSourceSnapshotCoverage(range:{from:string;to:string},markers:SourceSnapshotGeneration[],options:{minimumVersion?:number}={}):SourceSnapshotCoverage{const accepted=availableSourceSnapshotDays(range,markers,options),acceptedDates=new Set(accepted.map(marker=>marker.date)),expected:string[]=[];for(let day=range.from;day<=range.to;day=nextDay(day))expected.push(day);return{...range,acceptedFrom:accepted[0]?.date||null,acceptedTo:accepted.at(-1)?.date||null,acceptedDays:acceptedDates.size,expectedDays:expected.length,missingDays:expected.filter(day=>!acceptedDates.has(day))}}
export function restrictSourceSnapshotCoverage(coverage:SourceSnapshotCoverage,range:{from:string;to:string}):SourceSnapshotCoverage{const from=range.from>coverage.from?range.from:coverage.from,to=range.to<coverage.to?range.to:coverage.to,missing=new Set(coverage.missingDays),expected:string[]=[],accepted:string[]=[];for(let day=from;day<=to;day=nextDay(day)){expected.push(day);if(!missing.has(day))accepted.push(day)}return{from,to,acceptedFrom:accepted[0]||null,acceptedTo:accepted.at(-1)||null,acceptedDays:accepted.length,expectedDays:expected.length,missingDays:expected.filter(day=>missing.has(day))}}
