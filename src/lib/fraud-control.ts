...
type FraudEvaluationContext={conversions:FraudConversionInput[];now?:Date;maturityHours?:number;baselines:Record<string,number>};
export const compareFraudEvaluations=(a:FraudSourceEvaluation,b:FraudSourceEvaluation)=>b.fraudScore-a.fraudScore||b.qualityScore-a.qualityScore||a.key.localeCompare(b.key);
export function evaluateFraudSources(input:FraudEvaluationContext&{metrics:FraudMetricInput[]}):FraudSourceEvaluation[]{
  const grouped=new Map<string,FraudMetricInput>();
  for(const row of input.metrics)accumulateFraudMetric(grouped,row);
  return [...iterateFraudEvaluations(grouped,input)].sort(compareFraudEvaluations);
}
/** Consume compact groups one at a time; callers need not retain all expanded evaluations. */
export function* iterateFraudEvaluations(grouped:Map<string,FraudMetricInput>,input:FraudEvaluationContext):Generator<FraudSourceEvaluation>{
  ...
  const registrationsByKey=new Map<string,FraudConversionInput[]>();
  for(const row of canonicalRegistrations.values()){const key=evaluationKey(row),rows=registrationsByKey.get(key)||[];rows.push(row);registrationsByKey.set(key,rows)}
  for(const[key,seed]of grouped){
    const totals={clicks:seed.clicks,sois:seed.sois,firstSales:seed.firstSales,rebills:seed.rebills,coinEvents:seed.coinEvents,payout:seed.payout,revenue:seed.revenue};
    const registrations=registrationsByKey.get(key)||[],matureRegistrations=registrations.filter(row=>now-Date.parse(row.convertedAt)>=maturityMs);
