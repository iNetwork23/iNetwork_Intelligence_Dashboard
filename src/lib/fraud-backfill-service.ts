...
export async function runFraudConversionSync(now=new Date()){
  ...
  const reportResult=await refreshHistoryRange({store,from:window.from,to:window.to,includeConversions:false,loadConversions:source.loadConversions,loadReports:source.loadReports});
  const reportHasActivity=reportResult.metrics.some(row=>row.sois>0||row.first_sales>0||row.rebills>0||row.coin_spend>0);
  const result=await refreshConversionRange({store,from:window.from,to:window.to,loadConversions:source.loadConversions}),stored=await storedEvidence(window.from,window.to),parity=buildFraudBackfillParity({from:window.from,to:window.to,expected:{typeCounts:result.typeCounts,identityDigest:result.identityDigest},stored,reportHasActivity}),next=advanceFraudBackfillState(state,window,now,parity),saved=await client.from('sync_state').upsert({key:FRAUD_BACKFILL_KEY,value:next},{onConflict:'key'});
