...
type MetricRpcRow={affiliate_id:string;affiliate_name:string;offer_id:string;offer_name:string;campaign_id:string;campaign_name:string;offer_url_id:string;offer_url_name:string;clicks:number|string;sois:number|string;first_sales:number|string;rebills:number|string;coin_spend:number|string;payout:number|string;revenue:number|string;profit:number|string;traffic_mode?:string|null;click_metrics_eligible?:boolean};
...
// Individual daily facts carry traffic_mode; aggregated snapshots carry the
// conjunction of their input eligibility. Offer names cannot prove either.
const comparableMetricRow=(row:MetricRpcRow)=>row.click_metrics_eligible===undefined?row.traffic_mode==='tracked':row.click_metrics_eligible===true;
function reportRows(rows:MetricRpcRow[]){
  const base:ReportRow[]=[],events:ReportRow[]=[];
  for(const row of rows){
    const dimensions=columns(row);
    base.push({columns:dimensions,reporting:{click_metrics_eligible:comparableMetricRow(row)?1:0,total_click:number(row.clicks),cv:number(row.sois),payout:number(row.payout),revenue:number(row.revenue),profit:number(row.profit)}});
...
function aggregateMetricRows(rows:MetricRpcRow[]){const grouped=new Map<string,MetricRpcRow>();for(const row of rows){const key=[row.affiliate_id,row.offer_id,row.campaign_id,row.offer_url_id].join('\u0000'),current=grouped.get(key)||{...row,clicks:0,sois:0,first_sales:0,rebills:0,coin_spend:0,payout:0,revenue:0,profit:0,click_metrics_eligible:true};current.click_metrics_eligible=comparableMetricRow(current)&&comparableMetricRow(row);for(const metric of['clicks','sois','first_sales','rebills','coin_spend','payout','revenue','profit']as const)current[metric]=number(current[metric])+number(row[metric]);grouped.set(key,current)}return Array.from(grouped.values())}
const decodePortfolioRow=(row:PortfolioSnapshotRow):MetricRpcRow=>({click_metrics_eligible:row.ce===undefined?row.m==='tracked':row.ce===true&&(row.m===undefined||row.m==='tracked'),affiliate_id:row.a,affiliate_name:row.an,offer_id:row.o,offer_name:row.on,campaign_id:row.c,campaign_name:row.cn,offer_url_id:row.u,offer_url_name:row.un,clicks:row.cl,sois:row.cv,first_sales:row.fs,rebills:row.rb,coin_spend:row.cs,payout:row.p,revenue:row.r,profit:row.pr});
async function loadMetricRows(client:CacheClient,range:ReportingRange,preferRangeSnapshot=true){await assertBerlinReportingRange(client,range);const db=client as unknown as SupabaseClient;if(preferRangeSnapshot&&range.from){
 ...
 const marker=markerData?.value as{reportingVersion?:number;timezoneId?:number;version?:number;from?:string;to?:string;generation?:string}|undefined;
 if(marker?.reportingVersion===5&&marker.timezoneId===56&&marker.version===2&&marker.from===range.from&&marker.to===range.to&&typeof marker.generation==='string'&&marker.generation){
...
async function loadPortfolioDayRows(client:CacheClient,range:ReportingRange):Promise<{days:string[];byDay:Map<string,MetricRpcRow[]>}|undefined>{
 ...
 await assertBerlinReportingRange(client,range);
 ...
 const markers=new Map((markerData||[]).filter(item=>isBerlinDayMarker(item.value)).map(item=>{const value=item.value as{date?:string;generation?:string};return[value.date||'',value.generation||'']}));
