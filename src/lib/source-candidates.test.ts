...
const loadRows=vi.fn(),loadIndex=vi.fn(),loadFreshness=vi.fn();
vi.mock('./cached-evaluations',()=>({loadAffiliateSourceRowsRangeFromCache:(...a:unknown[])=>loadRows(...a),loadAffiliateActivityIndex:(...a:unknown[])=>loadIndex(...a),loadSourceSnapshotFreshness:(...a:unknown[])=>loadFreshness(...a)}));
...
beforeEach(()=>{vi.clearAllMocks();loadFreshness.mockResolvedValue({complete:true,availableDays:365,expectedDays:365,minDate:'2025-09-05',maxDate:'2026-09-04',generatedAt:'2026-09-04T10:00:00Z'});loadIndex.mockResolvedValue([]);maybeSingle.mockResolvedValue({data:null,error:null});upsert.mockResolvedValue({error:null})});
...
describe('buildSourceCandidatesSnapshot',()=>{
 it('walks the portfolio affiliates with system access, evaluates each with the cached readers and skips failing affiliates',async()=>{
  ...
  const snapshot=await buildSourceCandidatesSnapshot(range,{now:new Date('2026-09-04T12:00:00Z')});
