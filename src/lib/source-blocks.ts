...
export type SourceBlockInput={affiliateId:string;affiliateName:string;offerId:string;offerName:string;trafficMode:SourceTrafficMode;level:SourceBlockLevel;mainValue?:string|null;subValue?:string|null;reason?:string};
export type NormalizedSourceBlock={affiliateId:number;affiliateName:string;offerId:number;offerName:string;trafficMode:SourceTrafficMode;level:SourceBlockLevel;mainField:'source_id'|'adv1';mainValue:string|null;subField:'sub1'|'adv2';subValue:string|null;variables:EverflowBlockVariable[];reason:string};
...
const normalizedValue=(value:unknown)=>{if(value===undefined||value===null)return null;const text=String(value).trim();if(!text||text==='N/A'||text==='Ohne Source-ID'||text==='Ohne Sub-Source')return null;if(text.length>200)throw new Error('Quellenwert ist zu lang');return text};
