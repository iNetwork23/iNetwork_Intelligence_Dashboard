...
export function evaluateAutomation(config:AutomationConfiguration,metrics:AutomationVariantMetrics[],now=new Date()):AutomationEvaluation{
 const supportedConfiguration=['equal','champion_challenger'].includes(config.weights.mode as string)&&config.offers.every(offer=>offer.landingpages.every(landingpage=>['active','candidate','excluded'].includes(landingpage.selection as string)));
 if(!supportedConfiguration)return{evaluatedAt:now.toISOString(),action:{type:'hold',reasonCode:'unsupported_configuration'},progress:[],targetSlots:config.slots,writesPlanned:0,blockers:['Ausführbare Konfiguration enthält nicht unterstützte Enum-Werte.']};
