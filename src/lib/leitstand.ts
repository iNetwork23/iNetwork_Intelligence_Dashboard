...
export type LeitstandModel={range:{from:string;to:string};generatedAt:string;affiliates:number;affiliatesProcessed:number;coverageComplete:boolean;scopeRestricted?:boolean;maturityUnavailable?:number;losses:LeitstandRow[];winners:LeitstandRow[];counters:LeitstandCounters};
...
export function leitstandRow(row:SourceCandidate,index:BlockIndex,access?:AccessMetadata):LeitstandRow{
 const record=blockOf(row,index),state:LeitstandBlockState=record?record.status:'none';
 return{key:sourceCandidateBlockKey(row),href:sourceCandidateHref(row,LEITSTAND_RANGE),title:`${row.affiliate} · ${row.offer}`,affiliate:row.affiliate,offer:row.offer,source:sourceBlockLabel(row),action:row.action,severity:row.severity,reason:row.reason,profit:row.profit,sois:row.sois,clicks:row.clicks,leadStatus:row.leadStatus,block:{state,since:state==='active'?record!.effectiveAt:null,id:record&&(!access||sourceBlockInScope(record,access))?record.id:null},blockable:isBlockableCandidate(row)};
}
...
export function countLeitstand(rows:SourceCandidate[],index:BlockIndex,access?:AccessMetadata):LeitstandCounters{
 let activeBlocks=0,incidents=0;for(const record of index.values()){if(access&&!sourceBlockInScope(record,access))continue;if(record.status==='active')activeBlocks++;else if(record.status==='pending'||record.status==='error')incidents++}
 return{openKill:rows.filter(row=>isOpenKill(row,index)).length,activeBlocks,incidents};
}
export function buildLeitstand(snapshot:SourceCandidatesSnapshot|null,index:BlockIndex,limit=LEITSTAND_TOP_N,access?:AccessMetadata):LeitstandModel|null{
 if(!snapshot)return null;
 return{range:snapshot.range,generatedAt:snapshot.generatedAt,affiliates:snapshot.affiliates,affiliatesProcessed:snapshot.affiliatesProcessed,coverageComplete:snapshot.coverageComplete,...(snapshot.scopeRestricted?{scopeRestricted:true}:{}),...(snapshot.maturityUnavailable?{maturityUnavailable:snapshot.maturityUnavailable}:{}),losses:rankLosses(snapshot.rows,limit).map(row=>leitstandRow(row,index,access)),winners:rankWinners(snapshot.rows,limit).map(row=>leitstandRow(row,index,access)),counters:countLeitstand(snapshot.rows,index,access)};
}
...
export function describeRollup(model:Pick<LeitstandModel,'generatedAt'|'affiliates'|'affiliatesProcessed'|'coverageComplete'>&{maturityUnavailable?:number;scopeRestricted?:boolean}):{source:string;warning:string|null;maturityWarning:string|null}{
 const time=Number.isFinite(Date.parse(model.generatedAt))?berlinDateTime.format(new Date(model.generatedAt)):'unbekannt';
 if(model.scopeRestricted)return{source:`Rollup vom ${time} · Zugewiesener Bereich`,warning:model.coverageComplete?null:'Die Vollständigkeit der Quellenliste ist noch nicht bestätigt.',maturityWarning:model.maturityUnavailable?'Die Reifeprüfung ist noch nicht vollständig verfügbar.':null};
...
export async function loadLeitstand(access:AccessMetadata,now=new Date()):Promise<LeitstandView>{
 ...
 return{model:buildLeitstand(snapshot,index,LEITSTAND_TOP_N,access),failed,blockIndexUnavailable};
}
/** Cache identity includes every data scope; counts never use elevated access. */
export async function loadLeitstandCounters(access:AccessMetadata,now=new Date()):Promise<LeitstandCounters>{
 const range=leitstandRange(now);
 return unstable_cache(async()=>{const[snapshot,index]=await Promise.all([loadSourceCandidates(range,access),loadBlockIndex()]);return countLeitstand(snapshot?.rows??[],index,access)},['leitstand-counters-v2-scoped',range.from,range.to,scopeFingerprint(access)],{revalidate:LEITSTAND_COUNTERS_REVALIDATE_SECONDS,tags:['source-candidates','source-blocks']})();
}
