...
const blockOf=(row:SourceCandidate,index:BlockIndex)=>index.get(sourceCandidateBlockKey(row));
const isOpenKill=(row:SourceCandidate,index:BlockIndex)=>row.action==='ABSCHALTEN'&&!RUNNING.has(blockOf(row,index)?.status as SourceBlockRecord['status']);
...
export function leitstandRow(row:SourceCandidate,index:BlockIndex):LeitstandRow{
 ...
 return{key:sourceCandidateBlockKey(row),href:sourceCandidateHref(row,LEITSTAND_RANGE),title:`${row.affiliate} · ${row.offer}`,affiliate:row.affiliate,offer:row.offer,source:sourceBlockLabel(row),action:row.action,severity:row.severity,reason:row.reason,profit:row.profit,sois:row.sois,clicks:row.clicks,leadStatus:row.leadStatus,block:{state,since:state==='active'?record!.effectiveAt:null,id:record?.id??null}};
}
...
export async function loadLeitstand(access:AccessMetadata,now=new Date()):Promise<LeitstandView>{
 ...
 try{snapshot=await loadSourceCandidates(range,access)}catch(error){failed=true;console.error('Leitstand: Quellen-Rollup nicht lesbar',error)}
 if(snapshot){try{index=await loadBlockIndex()}catch(error){blockIndexUnavailable=true;console.error('Leitstand: Sperr-Index nicht lesbar',error)}}
