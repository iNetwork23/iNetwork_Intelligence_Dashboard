...
export function automationRuntimeDependencies(input:Hooks){const apiKey=process.env.EVERFLOW_API_KEY||'';return{
 ...
 applyRouting:async({config,evaluation,expectedFingerprint,beforeWrite}:{config:StoredAutomationConfiguration;evaluation:AutomationEvaluation;expectedFingerprint:string;beforeWrite:()=>Promise<void>})=>applyAutomationRouting({campaignId:config.campaignId,affiliateId:config.affiliateId,targetSlots:evaluation.targetSlots,expectedFingerprint,apiKey,beforeWrite}),
