...
export async function recordSourceBlockHistory(event:Omit<SourceBlockHistoryEvent,'id'|'at'>,store?:SecurityStore):Promise<void>{
 try{
  if(!ACTIONS.includes(event.action))throw new Error(`Unbekannte Historie-Aktion: ${String(event.action)}`);
  const record:SourceBlockHistoryEvent={...event,blockId:blockSegment(event.blockId),identityKey:text(event.identityKey,400),actorId:text(event.actorId,200),id:crypto.randomUUID(),at:new Date().toISOString()};
